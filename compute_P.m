function [P, P1, Pt1, PX, L]=compute_P(X, T, s_matrix, sigma2 ,outlier, iter)
[N, D] = size(X);
M = size(T, 1);
% s_matrix = ones(N, M) / M;
if D == 2
...
cp=exp(-iter/50);
P = squeeze(sum(P.^2,2));
% P = cp*squeeze(sum(P.^2,2))+s_matrix;
% P = s_matrix.*squeeze(sum(P.^2,2));
P = P/ksig+cp*s_matrix;
% P = exp(P).*s_matrix;
P = exp(P);
% P = exp(P)+exp(-iter/15)*s_matrix;

function Iout=tps_warp(Iin,Pos1,Pos2,mode)
...
k1=size(x,1);
k2=size(x,2);
points=zeros(k1*k2,3);
count=1;
for i=1:k1
    for j=1:k2
        points(count,:)=[xd(i,j) yd(i,j) 0];
        count=count+1;
    end
end
points=TPS3D(Pos2,Pos1,points);
count=1;
for i=1:k1
    for j=1:k2
        Tlocalx(i,j)=points(count,1);
        Tlocaly(i,j)=points(count,2);
        count=count+1;
    end
end
